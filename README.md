 # 📄 ДОМАШНЕЕ ЗАДАНИЕ №1: Системный анализ сервиса «Прокачу»

Добро пожаловать в репозиторий с материалами обследования и требованиями к мобильному приложению сервиса аренды велосипедов «Прокачу».

## 📑 Оглавление

1. 📄 Документ 1: Протокол встречи и анализ
   - [Протокол встречи](docs/01-meeting-protocol.md)
   - [Анализ протокола (метод трёх цветов)](docs/01a-protocol-analysis.md)
2. 📊 Документ 2: Систематизация данных и детализация процессов
   - [Систематизация данных](docs/02-data-systematization.md)
   - [Детализация бизнес-процессов (альтернативные сценарии, статусы, SLA)](docs/02a-business-processes-details.md)
3. [📋 Документ 3: Реестр требований](docs/03-requirements-registry.md)
4. [⚙️ Документ 4: Требования к системе (SRS)](docs/04-srs.md)
5. [📊 Диаграмма Use Case](diagrams/use-case.md)
6. [📊 Диаграмма Успешная аренда с автозавершением](diagrams/sequence-rental.md)
7. [📊 Диаграмма Жизненный цикл аренды](diagrams/state-machine-rental.md)
8. [🔌 API Контракт (OpenAPI / Swagger)](api/index.html) _(Интерактивная документация)_
9. [🎨 UI/UX Прототип (Figma)](#figma-prototype)

---

### 🎨 UI/UX Прототип <a id="figma-prototype"></a>

Ниже представлен интерактивный прототип мобильного приложения.
_(Здесь встроен фрейм Figma)_

<iframe style="border: 1px solid rgba(0, 0, 0, 0.1);" width="800" height="450" src="https://embed.figma.com/design/aGG1th64a9ZCIcRBaLh48u/Bike-Sharing-App-UI--Community-?node-id=0-1&embed-host=share" allowfullscreen></iframe>

---

**Исполнитель:** Оксана Абрамова  
**Дата:** 19 июля 2026 г.  
**Репозиторий:** [prokachu-requirements](https://github.com/ksiu999/procachu)
